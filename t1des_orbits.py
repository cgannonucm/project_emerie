#!/usr/bin/env python3
"""
Orbital distance over time for Galacticus subhalos.

Computes r = sqrt(x^2 + y^2 + z^2) from satellite position vectors
and plots the orbital history for each sample subhalo.
"""
...
def plot_orbital_distance(ax, galacticus_hdf5, halo_id, cosmo, tree_index=0):
    """
    Plot the orbital distance r(t) for a subhalo over time.
    ...
    # Satellite position in Mpc -> convert to kpc
    x = data['satellitePositionX'] * 1e3
    y = data['satellitePositionY'] * 1e3
    z = data['satellitePositionZ'] * 1e3
...
if __name__ == '__main__':
    ...
    with h5py.File(gal_file, 'r') as gout:
        for ax, halo_id in zip(axes.flatten(), halo_ids):
            plot_orbital_distance(ax, gout, halo_id, cosmo)
            ax.set_title(f'Halo {halo_id}')

    fig.suptitle('Subhalo Orbital Distance Histories', y=1.01)
    fig.tight_layout()

    os.makedirs('out/plots', exist_ok=True)
    fig.savefig('out/plots/t1des_orbits.png', bbox_inches='tight')
    plt.show()
