#!/usr/bin/env python3
"""
Orbital velocity over time for Galacticus subhalos.

Computes v = sqrt(vx^2 + vy^2 + vz^2) from satellite velocity vectors
and plots the velocity history for each sample subhalo.
"""
...
def plot_orbital_velocity(ax, galacticus_hdf5, halo_id, cosmo, tree_index=0):
    """
    Plot the orbital velocity v(t) for a subhalo over time.
    ...
    vx = data['satelliteVelocityX']
    vy = data['satelliteVelocityY']
    vz = data['satelliteVelocityZ']
...
if __name__ == '__main__':
    ...
    with h5py.File(gal_file, 'r') as gout:
        for ax, halo_id in zip(axes.flatten(), halo_ids):
            plot_orbital_velocity(ax, gout, halo_id, cosmo)
            ax.set_title(f'Halo {halo_id}')

    fig.suptitle('Subhalo Orbital Velocity Histories', y=1.01)
    fig.tight_layout()

    os.makedirs('out/plots', exist_ok=True)
    fig.savefig('out/plots/t1des_velocity.png', bbox_inches='tight')
    plt.show()
